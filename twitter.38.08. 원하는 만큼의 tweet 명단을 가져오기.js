...
const twitter = {
    ...
    getTweets: async (username, count = 10) => {
        ...
        let tweetArrays = await page.$$('#stream-items-id > li');     // 첫번째 tweet list array 갯수 추출(여기서는 21개)
        let tweetLists = [];       
        let lastTweetsArrayLength = 0;
        while(tweetArrays.length < count) {
            
            await page.evaluate(`window.scrollTo(0, document.body.scrollHeight)`);  // scrollBar를 이용하여 페이지를 지정하는모드
            await page.waitFor(3000);     // 3초를 기다린다
            tweetArrays = await page.$$('#stream-items-id > li');
            if(lastTweetsArrayLength == tweetArrays.length) break;
            lastTweetsArrayLength = tweetArrays.length;
        }
        ...
        console.log('최초 count를 빼기전  값은:', tweetLists);
        tweetLists = tweetLists.slice(0, count);
        console.log('새로운 tweet lists 갯수', tweetLists);
        return tweetLists;
